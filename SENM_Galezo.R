...
{
  #note- manually updated 2014 date error in data files
  updates$observation_date<-as.Date(updates$observation_date, format="%d-%b-%y")
  survey_general$observation_date<-as.Date(survey_general$observation_date, format="%m/%d/%Y")
  survey_dolphins$observation_date<-as.Date(survey_dolphins$observation_date, format="%m/%d/%Y")
  
  #update ids
  
  update_ids<-updates[which(updates$update_type %in% c("update_id", "delete_id")),]
  
  survey_dolphins<-survey_dolphins[!duplicated(survey_dolphins[,c("observation_id", "dolphin_id", "unknown_dolphin_alias")]),]
  
  survey_dolphins<-merge(survey_dolphins, update_ids, 
                         by.x=c("observation_id", "observation_date", "dolphin_id", "unknown_dolphin_alias"),
                         by.y=c("old_observation_id", "observation_date", "old_dolphin_ID", "old_unknown_dolphin_alias"), 
                         all.x = TRUE, all.y=TRUE)
  
  
  #add dolphins
  
  add_ids<-updates[updates$update_type=="add_id",]
  
  names(add_ids)[which(names(add_ids)=="old_observation_id")]<-"observation_id"
  names(add_ids)[which(names(add_ids)=="observation_start_time")]<-"observation_start_time.x"
  
  nadd<-nrow(add_ids)
  
  survey_dolphins[(nrow(survey_dolphins)+1):(nrow(survey_dolphins)+nadd),]<-NA
  
  merge_columns<-names(add_ids)[which(names(add_ids) %in% names(survey_dolphins))]
  
  survey_dolphins[(nrow(survey_dolphins)-nadd+1):(nrow(survey_dolphins)),merge_columns]<-add_ids[,merge_columns]
  
  #change beyond 10 m to "f" from NA
  
  survey_dolphins$beyond_10_meters[which(is.na(survey_dolphins$beyond_10_meters))]<-"f"
  
  #update dolphin_id and dolphin_certainty
  
  survey_dolphins$dolphin_id[!is.na(survey_dolphins$update_type)]<-survey_dolphins$new_dolphin_ID[!is.na(survey_dolphins$update_type)]
  
  survey_dolphins$dolphin_id_certainty[!is.na(survey_dolphins$update_type)]<-survey_dolphins$new_dolphin_certainty[!is.na(survey_dolphins$update_type)]
  
  #format and subset dates
  
  new_db_dolphins$observation.date<-as.Date(new_db_dolphins$observation.date, format="%Y-%m-%d")
  
  new_db_2017<-new_db_dolphins[which(new_db_dolphins$observation.date>"2017-08-01"),]
  
  ncur<-nrow(survey_dolphins)+1
  nfin<-nrow(survey_dolphins)+nrow(new_db_2017)
  
  survey_dolphins[ncur:nfin,]<-NA
  
  #add necessary columns
  survey_dolphins$observation_date[ncur:nfin]<-new_db_2017$observation.date
  survey_dolphins$new_observation_id[ncur:nfin]<-new_db_2017$observation.id
  survey_dolphins$observation_id[ncur:nfin]<-new_db_2017$observation.id
  survey_dolphins$dolphin_id[ncur:nfin]<-new_db_2017$dolphin.id
  survey_dolphins$dolphin_id_certainty[ncur:nfin]<-new_db_2017$dolphin.id.certainty
  survey_dolphins$beyond_10_meters[ncur:nfin]<-new_db_2017$beyond.ten.meters.ind
  survey_dolphins$observation_start_time.x[ncur:nfin]<-new_db_2017$observation.start.time
  
  #remove empty dolphin_id rows at the end
  survey_dolphins$dolphin_id[survey_dolphins$dolphin_id==""]<-NA
  all_surveys<-survey_dolphins[complete.cases(survey_dolphins$dolphin_id),]
  
  #remove resights, get one survey per dolphin per day, and add gps information to all_surveys
  
  names(new_db_general)<-gsub("\\.", "_", names(new_db_general))
  
  names(new_db_general)<-gsub("departure", "depart", names(new_db_general))
  
  #add gps data to new_db_general
  
  gps2017$date<-as.Date(gps2017$date, format="%d-%b-%Y")
  new_db_general$observation_date<-as.Date(new_db_general$observation_date, format="%Y-%m-%d")
  
  new_db_general<-merge(new_db_general, gps2017, 
                        by.x=c("observation_date", "waypoint"), 
                        by.y=c("date", "name"), all.x = TRUE, all.y=FALSE)
  
  names(new_db_general)[which(names(new_db_general)=="lon")]<-"gps_east"
  names(new_db_general)[which(names(new_db_general)=="lat")]<-"gps_south"
  
  keep_names<-names(new_db_general)[names(new_db_general) %in% names(survey_general)]
  
  survey_general<-rbind(survey_general[,keep_names], new_db_general[,keep_names])
  
  survey_general_lookup<-survey_general[!duplicated(survey_general$observation_id),]
  
  all_surveys<-merge(all_surveys, survey_general_lookup, by="observation_id", all.x=TRUE, all.y=FALSE)
  
  all_surveys<-all_surveys[which(all_surveys$primary_depart_reason!="RESIGHT"
                                 & all_surveys$secondary_depart_reason!="RESIGHT"),]
  
  #change to last point per day
  
  all_surveys<-all_surveys[order(all_surveys$observation_start_time.x, decreasing=TRUE),]
  
  all_surveys<-all_surveys[!duplicated(all_surveys[,c("observation_date.x", "dolphin_id")]),]
  
  ##clean up gps data and add updates
  
  fix_gps<-updates[updates$update_type=="update_gps",]
  
  all_surveys[which(all_surveys$observation_id %in% fix_gps$old_observation_id),]
  
  fix_gps_south<-fix_gps[!is.na(fix_gps$new_gps_south), c("old_observation_id", "new_gps_south")]
  fix_gps_east<-fix_gps[!is.na(fix_gps$new_gps_east), c("old_observation_id", "new_gps_east")]
  
  for (i in 1:nrow(fix_gps_east)){
    all_surveys$gps_east[which(all_surveys$observation_id==fix_gps_east[i,1])]<-fix_gps_east[i,2]
  }
  
  for (i in 1:nrow(fix_gps_south)){
    all_surveys$gps_south[which(all_surveys$observation_id==fix_gps_south[i,1])]<-fix_gps_south[i,2]
  }
  
  all_surveys$gps_east<-as.numeric(all_surveys$gps_east)
  all_surveys$gps_south<-as.numeric(all_surveys$gps_south)
  
  #set where = -25. and 113 to blank
  
  all_surveys$gps_east[all_surveys$gps_east==113]<-NA
  all_surveys$gps_south[all_surveys$gps_south==-25]<-NA
  
  all_surveys<-all_surveys[complete.cases(all_surveys$gps_south, all_surveys$gps_east),]
  
  all_surveys<-all_surveys[,c("dolphin_id", "observation_id", "observation_date.x", 
                              "dolphin_id_certainty", "beyond_10_meters",
                              "gps_south", "gps_east")]
...
all_surveys<-all_surveys[which(all_surveys$dolphin_id_certainty!="UNCERTAIN" 
                                & all_surveys$dolphin_id_certainty!="LOW"
                                & all_surveys$dolphin_id_certainty!="ONE OF TWO"
                                & all_surveys$dolphin_id!="ANYDOLP"
                                & all_surveys$dolphin_id!="BIRD"
                                & all_surveys$dolphin_id!="FISH"  
                                & all_surveys$dolphin_id!="UNKNOWN"
                                & all_surveys$beyond_10_meters!="t"
                                & all_surveys$beyond_10_meters!="True"),]

#add age and weaning date from life_history

life_history_lookup<-life_history[!duplicated(life_history$dolphin_id),]

all_surveys<-merge(all_surveys, life_history_lookup[,c("dolphin_id", "birth_date", "weaning_date")], by="dolphin_id", all=FALSE)

all_surveys$life_stage<-ifelse(all_surveys$observation_date.x>=all_surveys$weaning_date, "postweaning", "preweaning")

all_surveys$age<-as.numeric((all_surveys$observation_date.x-all_surveys$birth_date)/365.25)
...
#removing a few known outlier points (need to check and update these later) 

killobs<-outliers$observation_id

all_surveys<-all_surveys[!all_surveys$observation_id %in% killobs,]

all_surveys<-all_surveys[complete.cases(all_surveys$gps_south),]
